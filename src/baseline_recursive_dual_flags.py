"""
Dual-target business-constrained experiment -- FIXED + PROMO/TET FLAGS
====================================================================
Scenarios:
...
Important fixes vs baseline_recursive_dual.py:
  ...
  6) Adds USE_PROMO / USE_TET flags at the top of the file.
"""
import os
import warnings
...
# Feature switches
# -----------------------------------------------------------------------------
# Toggle these to run the same 5 business-constrained scenarios with/without
# projected promotion features and Tet-related calendar features.
#
# USE_PROMO=False removes: promo_active, promo_count, max_discount,
#                         days_into_promo, days_until_promo_end
# USE_TET=False removes: days_to_tet, is_tet_week
#
# The output file names include the flag values to avoid overwriting submissions.
USE_PROMO = False
USE_TET = True
...
PROMO_FEATURE_COLS = ["promo_active", "promo_count", "max_discount", "days_into_promo", "days_until_promo_end"]
TET_FEATURE_COLS = ["days_to_tet", "is_tet_week"]
CONFIG_TAG = f"promo{int(USE_PROMO)}_tet{int(USE_TET)}"
...
def add_cal_row(df: pd.DataFrame, idx: int) -> pd.DataFrame:
    ...
    if USE_TET:
        dtt = days_to_tet(dt)
        df.loc[idx, "days_to_tet"] = dtt
        df.loc[idx, "is_tet_week"] = int(dtt <= TET_WINDOW_DAYS)
    return df
...
def build_features_vec(df_in: pd.DataFrame, target_col: str, promo_cal: pd.DataFrame, burnin: int = BURN_IN) -> Tuple[pd.DataFrame, np.ndarray, List[str]]:
    ...
    if USE_PROMO:
        df = df.merge(promo_cal[["Date"] + PROMO_FEATURE_COLS], on="Date", how="left")
        for col in PROMO_FEATURE_COLS:
...
def save_submission_for_scenario(name: str, func, sales: pd.DataFrame, test_raw: pd.DataFrame, test_sorted: pd.DataFrame, promo_cal: pd.DataFrame, pcal_dict: Dict[pd.Timestamp, dict]) -> str:
    ...
    path = os.path.join(OUTPUT_DIR, f"submission_dual_{name.lower()}_{CONFIG_TAG}.csv")
...
def main():
    ...
    print(f"Feature flags: USE_PROMO={USE_PROMO}, USE_TET={USE_TET} ({CONFIG_TAG})")
