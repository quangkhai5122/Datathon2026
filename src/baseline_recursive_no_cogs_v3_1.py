...
# Feature windows
LAG_WINDOWS = [1, 2, 5, 7, 14, 28, 30, 60, 90, 180, 182, 364, 365, 371]
...
print(f"\n{'='*80}")
print("BASELINE RECURSIVE v3.1 — CLEANUP PATCH")
print(f"{'='*80}")
...
def load_data() -> Tuple[pd.DataFrame, pd.DataFrame, int]:
    """Load and validate training and test data, infer horizon."""
    ...
    # Infer Horizon
    assert test['Date'].nunique() == len(test), "Test dates are not unique. Cannot infer simple horizon."
...
def prepare_train_features_vectorized(train_df: pd.DataFrame, burnin: int = BURN_IN_DAYS) -> Tuple[pd.DataFrame, np.ndarray, List[str]]:
    ...
    # ---------------------------------------------------------
    # B. ROLLING STATISTICS & EXPANDING
    # ---------------------------------------------------------
...
def engineer_features_for_row(history_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Exact parity with vectorized training.
    """
    df = history_df.copy()
    df = add_calendar_features(df)
    ...
    # ---------------------------------------------------------
    # C. RECENCY-WEIGHTED FEATURES (EWMA) EXACT PARITY
    # ---------------------------------------------------------
    for span in EWMA_WINDOWS:
        if idx >= 1:
            alpha = 2 / (span + 1)
            # Mathematical recursive update
            # EWMA_t = alpha * Y_{t-1} + (1 - alpha) * EWMA_{t-1}
            # For exact parity, if idx-1 was the first observation, EWMA_0 = Y_0
            y_prev = df.loc[idx - 1, 'Revenue']
            if idx == 1:
                df.loc[idx, f'ewma_{span}'] = y_prev
            else:
                ewma_prev = df.loc[idx - 1, f'ewma_{span}']
                # Fallback if ewma_prev is NaN for some reason
                if pd.isna(ewma_prev):
                    df.loc[idx, f'ewma_{span}'] = y_prev
                else:
                    df.loc[idx, f'ewma_{span}'] = alpha * y_prev + (1 - alpha) * ewma_prev
        else:
...
def train_model(X_df: pd.DataFrame, y: np.ndarray) -> object:
    ...
    # LightGBM handles NaNs organically; no fillna needed.
    model.fit(X_df, y)
    return model


# ============================================================================
# 5. DYNAMIC HORIZON-ALIGNED ROLLING-ORIGIN BACKTEST
# ============================================================================

def rolling_origin_backtest(train_df: pd.DataFrame, horizon_days: int) -> Dict:
    """
    Dynamic backtest based on inferred horizon.
    """
    print(f"\n{'='*80}")
    print(f"DYNAMIC HORIZON-ALIGNED BACKTEST ({horizon_days} DAYS PER FOLD)")
    print(f"{'='*80}")
    
    # Build folds backwards from the end of history
    ...
    for fold_def in folds_def:
        ...
        # We must initialize history WITH EWMA correctly.
        # To do this cleanly, we'll just run the full history through vectorize, or we can just calculate EWMA iteratively.
        # Wait, if we use engineer_features_for_row, it expects EWMA of previous row to exist.
        # So we should seed the history with the vectorized features for the last train row!
        
        # Recalculate full features on train_fold just to have perfect recent row state
        history = train_fold[['Date', 'Revenue']].copy()
        
        # Calculate full history EWMA
        for span in EWMA_WINDOWS:
            history[f'ewma_{span}'] = history['Revenue'].ewm(span=span, adjust=False).mean()
        ...
        for val_idx in range(len(val_fold)):
            ...
            # Initialize EWMA columns for the new row to nan
            for span in EWMA_WINDOWS:
                new_row[f'ewma_{span}'] = np.nan
...
def save_submission(test_df: pd.DataFrame, predictions: np.ndarray) -> str:
    ...
    output_file = os.path.join(OUTPUT_DIR, 'submission_no_cogs_v3_1.csv')
